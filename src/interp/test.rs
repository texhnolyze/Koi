...
fn vec_equality() {
    assert_eq!(
        Value::Vec(Rc::new(RefCell::new(vec![Value::Num(1.0)]))),
        Value::Vec(Rc::new(RefCell::new(vec![Value::Num(1.0)])))
    );
}
...
fn dict_equality() {
    ...
    assert_eq!(
        Value::Dict(Rc::new(RefCell::new(a))),
        Value::Dict(Rc::new(RefCell::new(b)))
    );
}
...
fn scopes() {
    assert_eq!(
        output(
            "let name = 'ampere' print(name) {let name = 'thomas the dank engine' print(name)} print(name)",
            None
        ),
...
fn basic_output() {
    assert_eq!(
        output("echo xyz > /tmp/ampere ; cat /tmp/ampere", None),
        "xyz\n".to_string()
    );
}
...
fn assignment() {
    assert_eq!(
        output("let x = \"ampere\" x = \"xyz\" print(x)", None),
        "xyz\n".to_string()
    );
}
...
fn interp() {
    assert_eq!(
        output("let name = \"ampere\" print(\"x{name}x\")", None),
        "xamperex\n".to_string()
    );
    assert_eq!(
        output("let name = \"ampere\" print(\"{name}\")", None),
        "ampere\n".to_string()
    );
    assert_eq!(
        output("let name = \"ampere\" print(\"X{name}Y{name}Z\")", None),
        "XampereYampereZ\n".to_string()
    );
}
...
fn vecs_are_refs() {
    assert_eq!(
        output("let x = [1, 2, 3] let y = x y[0] = 99 print(x)", None),
        "[99, 2, 3]\n".to_string()
    );
}
...
fn dicts_are_refs() {
    assert_eq!(
        output(
            "let x = {a: 1, b: 2, c: 3} let y = x y['a'] = 99 y.b=55 print(x.a, x.b, x.c)",
            None
        ),
        "99 55 3\n".to_string()
    );
...
fn if_else_if_else_stmt() {
    assert_eq!(
        output("if false {print(1)} else if true {print(2)} else {print(3)}", None),
        "2\n".to_string()
    );
}
...
fn exported_var() {
    ...
    assert_eq!(
        output("exp let AMPERE = 123\nenv | grep AMPERE", None),
        "AMPERE=123\n".to_string()
    );
}
...
fn continue_stmt() {
    assert_eq!(
        output("for i in 0..=2 {if i == 1 {continue} print(i)}", None),
        "0\n2\n".to_string()
    );
}
...
fn break_stmt() {
    assert_eq!(
        output("for i in 0..=2 {if i == 1 {break} print(i)}", None),
        "0\n".to_string()
    );
}
...
fn close_scope_escaping_func() {
    assert_eq!(
        output("fn p() {let i = 10 return} p() print(i)", None),
        "nil\n".to_string()
    );
}
...
fn close_scope_escaping_loop() {
    assert_eq!(
        output("while true { let i = 10 break } print(i)", None),
        "nil\n".to_string()
    );
}
...
fn closure() {
    assert_eq!(
        output("let f { let i = 55 f = fn() { return i } } print(f())", None),
        "55\n".to_string()
    );
}
...
fn slices() {
    assert_eq!(
        output(
            "let x = [1 2 3 4 5 6] let y = x[0..5] y[0] = 99 print(x) print(y)",
            None
        ),
        "[1, 2, 3, 4, 5, 6]\n[99, 2, 3, 4, 5]\n".to_string()
    );
...
fn native_string() {
    assert_eq!(
        output("let x = [1 2 3].string() print(x + '!')", None),
        "[1, 2, 3]!\n".to_string()
    );
}
...
fn native_parse_json() {
    assert_eq!(
        output("print('[\"xyz\",\"b\"]'.parseJson()[0])", None),
        "xyz\n".to_string()
    );
}
...
fn native_replace() {
    assert_eq!(
        output("print('---xyz---'.replace('xyz', 'abc'))", None),
        "---abc---\n".to_string()
    );
}
...
fn native_map() {
    assert_eq!(
        output("print([2 4 6].map(fn(i){return i^2}))", None),
        "[4, 16, 36]\n".to_string()
    );
}
...
fn native_filter() {
    assert_eq!(
        output("print([1 2 3 4 5 6].filter(fn(i){return i%2==0}))", None),
        "[2, 4, 6]\n".to_string()
    );
}
...
fn native_for_each() {
    assert_eq!(
        output("let i = 0 let x = [1 2 3 4 5 6] x.forEach(fn(_){i+=1}) print(i)", None),
        "6\n".to_string()
    );
}
...
fn native_clone_vec() {
    assert_eq!(
        output("let x = [1] let y = x.clone() y[0] = 99 print(x)", None),
        "[1]\n".to_string()
    );
}
...
fn native_clone_dict() {
    assert_eq!(
        output("let x = {a:1} let y = x.clone() y.a = 99 print(x)", None),
        "{a: 1}\n".to_string()
    );
}
...
fn native_matches() {
    assert_eq!(
        output("print('00AAA'.matches('\\d\\d\\w\\{3}'))", None),
        "true\n".to_string()
    );
}
...
fn native_find() {
    assert_eq!(
        output("print('01 23'.find('(\\d)(\\d)'))", None),
        "[['01', '0', '1'], ['23', '2', '3']]\n".to_string()
    );
}
...
fn native_vec_remove() {
    assert_eq!(
        output("let x = [1 2] print(x.remove(0)) print(x)", None),
        "1\n[2]\n".to_string()
    );
}
...
fn native_dict_remove() {
    assert_eq!(
        output("let x = {a: 1 b: 2} print(x.remove('a')) print(x)", None),
        "1\n{b: 2}\n".to_string()
    );
}
...
fn native_string_contains() {
    ...
    assert_eq!(
        output("print('abcXYXdef'.contains('XXX'))", None),
        "false\n".to_string()
    );
}
