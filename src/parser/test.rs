...
fn parses_assignments() {
    for (source, want) in &[
        (
            "foo=5",
            Expr::Set("foo".to_owned(), Box::new(Expr::Literal(Value::Num(5.0)))),
        ),
        (
            "foo.bar=5",
            Expr::SetField {
...
#[test]
fn parses_return() {
    assert_eq!(parse("return"), vec![Stmt::Return(None),]);

    assert_eq!(
        parse("return 1"),
        vec![Stmt::Return(Some(Expr::Literal(Value::Num(1.0)))),]
    );
    ...
    assert_eq!(
        ...
        vec![
            Stmt::Return(None),
            Stmt::Cmd(Cmd::Atom(vec![vec![Expr::Literal(Value::String("1".to_owned())),]])),
        ]
    );
}
...
#[test]
fn parses_continue() {
    assert_eq!(parse("continue"), vec![Stmt::Continue,]);
}

#[test]
fn parses_break() {
    assert_eq!(parse("break"), vec![Stmt::Break,]);
}

#[test]
fn parses_import() {
    assert_eq!(parse("import \"foobar\""), vec![Stmt::Import("foobar".to_owned()),]);
}
...
fn parses_if() {
    assert_eq!(
        ...
        vec![Stmt::If {
            ...
            then_do: Box::new(Stmt::Block(vec![Stmt::Cmd(Cmd::Atom(vec![vec![Expr::Literal(
                Value::String("cmd_if_true".to_owned())
            ),]])),])),
            else_do: None,
        }]
    );
...
fn parses_if_with_else() {
    assert_eq!(
        ...
        vec![Stmt::If {
            ...
            else_do: Some(Box::new(Stmt::Block(vec![Stmt::Cmd(Cmd::Atom(vec![vec![
...
fn parses_cmd_semicolon() {
    assert_eq!(
        parse("cmd1 ; cmd2"),
        vec![Stmt::Cmd(Cmd::Op(
            Box::new(Cmd::Atom(vec![vec![Expr::Literal(Value::String("cmd1".to_owned())),]])),
            CmdOp::Seq,
            Box::new(Cmd::Atom(vec![vec![Expr::Literal(Value::String("cmd2".to_owned())),]])),
        ))]
    );
}
...
fn parses_lambda() {
    assert_eq!(
        ...
        vec![Stmt::Expr(Expr::Call {
            ...
            args: vec![Expr::Lambda(Func::User {
                name: None,
                params: vec![],
                body: Box::new(Stmt::Block(vec![])),
                captured_env: None,
...
fn parses_block() {
    assert_eq!(
        parse("{nop()}"),
        vec![Stmt::Block(vec![Stmt::Expr(Expr::Call {
...
fn parses_range_complex_exprs() {
    assert_eq!(
        parse_expression("v.len()..=[1,2,3].len()+1"),
        Expr::Range {
            l: Box::new(Expr::Call {
                func: Box::new(Expr::GetField {
